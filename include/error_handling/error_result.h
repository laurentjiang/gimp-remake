...
class Result {
  public:
     ...
     */
    Result(ErrorCode code, const std::string& context = "") : data_(ErrorInfo(code, context)) {}
...
class Result<void> {
  public:
     ...
     * @param context Additional context (optional)
     */
    Result(ErrorCode code, const std::string& context = "") : error_(ErrorInfo(code, context)) {}
     ...
     * @return Reference to the error information
     */
    const ErrorInfo& Error() const { return error_.value(); }
